...
public class CityFactory implements IWorldObjectFactory<City>
{
	...
	{
		public final int soilKernelSize;
		public final int resourceKernelSize;
		 ...
		 * @param seed
		 */
		public CityAttributes(double density, int minPop, int maxPop, double idealDistance, int soilKernelSize, int resourceKernelSize, int seed)
		{
			this.density = density;
			this.idealDistance = idealDistance;
			this.minPopulation = minPop;
			this.maxPopulation = maxPop;
			this.soilKernelSize = soilKernelSize;
			this.resourceKernelSize = resourceKernelSize;
			this.seed = seed;
		}

		public CityAttributes(CityAttributes copyAttributes)
		{
			this(copyAttributes.density, copyAttributes.minPopulation, copyAttributes.maxPopulation, copyAttributes.idealDistance,
					copyAttributes.soilKernelSize, copyAttributes.resourceKernelSize, copyAttributes.seed);
		}
	}
	...
	public static Collection<City> create(Terrain terrain, CityAttributes attributes)
	{
		...
		Map<ResourceType,Double> resources = null;
		
		Set<ResourceType> allResources = terrain.getResources();
		...
		for (int row = 0; row < heightmap.rows(); row++)
		{
			for (int col = 0; col < heightmap.cols(); col++)
			{
				...
				int averageSoilKernelSize = attributes.soilKernelSize;
				int averageResourceKernelSize = attributes.resourceKernelSize;
				double soil = soilQuality.sumDataOverArea(row, col, averageSoilKernelSize).getData()
						/ (averageSoilKernelSize * averageSoilKernelSize);
				if (h > 0 && soil * attributes.density > rval)
				{
					resources = new HashMap<ResourceType,Double>();
					for(ResourceType resType : allResources)
					{
						// calculate the resource mean value over a certain area
						double resMeanValue = terrain.getResourceMap(resType).sumDataOverArea(row, col, averageResourceKernelSize).getData();
						resMeanValue /= averageResourceKernelSize * averageResourceKernelSize;
						
						resources.put(resType, resMeanValue);
					}
					
					City newCity = new City(new GridCellInteger(ID), row, col, heightmap.getDataAt(row, col), resources);
	 ...
	 * 
	 * @param attributse min pop, max pop, ideal distance between two cities so that the connection can be fully utilised. anything closer results
	 *            also in full utilisation.
	 */
	public static void computeInitialSize(City c, CityAttributes attributes)
	{
		...
		// TODO: calculate correction formula for exponentially distributed population sizes
		double correction = 1;
	 ...
	 * @return
	 */
	public static ComparableGrid2D<Double> createPopulationDensityGrid(final ComparableGrid2D<Double> heightmap, Collection<City> cities,
